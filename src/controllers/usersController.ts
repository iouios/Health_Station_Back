...
export const editProfile = async (req: RequestWithToken, res: Response) => {
  try {
    const { username, firstname, lastname, phone, rolename } = req.body;
    const userId = req.params.id;

    const currentProfileQuery = `
            SELECT username, firstname, lastname, phone, rolename
            FROM master
            WHERE id = $1
        `;
    ...
    const updatedProfile = {
      ...
      rolename: rolename || currentProfile.rolename, // Add rolename here
    };

    const updateProfileQuery = `
            UPDATE master
            SET username = $1, firstname = $2, lastname = $3, phone = $4, rolename = $5
            WHERE id = $6
            RETURNING *
        `;
    const values = [
      ...
      updatedProfile.rolename, // Include rolename in the update query
      userId,
    ];
